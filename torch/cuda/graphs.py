...
class CUDAGraph(torch._C._CUDAGraph):
    ...
    def set_info_files(self, dependency_file: str, func_param_file: str, node_file: str, mf_file: str):
        r"""Set the file paths for the graph information files.

        Arguments:
            dependency_file (required): Path to the dependency file.
            func_param_file (required): Path to the function parameter file.
            node_file (required): Path to the node file.
            mf_file (required): Path to the memory footprint file.
        """
        return super().set_info_files(dependency_file, func_param_file, node_file, mf_file)
    
    def set_model(self, model_name: str):
        r"""Set the model name for the graph.
    ...
    def capture_begin(self, pool=None, capture_error_mode="global", load_graph: bool = False):
        ...
        """  # noqa: B950
        super().capture_begin(pool=pool, capture_error_mode=capture_error_mode, load_graph=load_graph)
...
class graph:
    ...
    def __enter__(self):
        ...
        self.cuda_graph.capture_begin(
            *self.pool, capture_error_mode=self.capture_error_mode, load_graph=self.load_graph
        )
