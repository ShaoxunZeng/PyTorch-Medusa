...
void CUDAPluggableAllocator::getAllMallocAddrs(std::vector<uint64_t> **malloc_addrs) {
  TORCH_CHECK(
      false,
      "CUDAPluggableAllocator does not yet support getAllMallocAddrs.");
}

void CUDAPluggableAllocator::shutdownFree() {
  TORCH_CHECK(
      false,
      "shutdownFree does not yet support shutdownFree.");
}

void CUDAPluggableAllocator::turnonFree() {
  TORCH_CHECK(
      false,
      "turnonFree does not yet support turnonFree.");
}

void CUDAPluggableAllocator::stopTraceMallocFree() {
  TORCH_CHECK(
      false,
      "CUDAPluggableAllocator does not yet support stopTraceMallocFree.");
}
