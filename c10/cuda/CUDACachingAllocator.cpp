...
// 6UL << 30; // "large" allocations may be packed in 20 MiB blocks
namespace Native {
  ...
  BlockPool& get_pool(size_t size, cudaStream_t stream) {
    ...
    if (C10_UNLIKELY(captures_underway)) {
      ...
      if (it0 != stream_to_pool_map.end()) {
        ...
        // printf("size %ld, pool %lld %lld\n", size, it1->first.first, it1->first.second);
        if (size <= kSmallSize) {
 ...
 public:
  ...
  void malloc(void** devPtr, int device, size_t size, cudaStream_t stream) {
    ...
    *devPtr = (void*)block->ptr;
    if (trace_malloc_free) {
      SERVERLESS_LOG("%ld malloc %p, size %ld, stream %p\n", malloc_addrs.size(), *devPtr, size, stream);
      malloc_addrs.push_back((uint64_t)*devPtr);
    }
    const c10::impl::PyInterpreter* interp = c10::impl::GPUTrace::get_trace();
    if (C10_UNLIKELY(interp)) {
  ...
  void free(void* ptr) {
    ...
    if (trace_malloc_free) {
      SERVERLESS_LOG("free %p\n", ptr);
    }
    if (no_free) {
      SERVERLESS_LOG("skip free %p\n", ptr);
      return;
    }
    Block* block = get_allocated_block(ptr, true /* remove */);
    if (!block) {
      // turn off the double free check
      // since free could be reorder if load graph and replay malloc free
      // and after capture_end, may free the same ptr again duirng capture
  ...
  void getAllMallocAddrs(std::vector<uint64_t> **out_malloc_addrs) override {
    *out_malloc_addrs = &this->malloc_addrs;
  }

  void shutdownFree() override {
    no_free = true;
  }

  void turnonFree() override {
    no_free = false;
  }

  void stopTraceMallocFree() override {
    trace_malloc_free = false;
  }
