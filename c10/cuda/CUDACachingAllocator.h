...
#undef NDEBUG
#ifndef NDEBUG
#define SERVERLESS_LOG(format, ...) do { \
    printf(format, ##__VA_ARGS__); \
    fflush(stdout); \
} while (0)
#else
#define SERVERLESS_LOG(format, ...)
#endif
...
class CUDAAllocator : public Allocator {
 public:
  ...
  virtual void getAllMallocAddrs(std::vector<uint64_t> **malloc_addrs) = 0;
  virtual void shutdownFree() = 0;
  virtual void turnonFree() = 0;
  virtual void stopTraceMallocFree() = 0;
...
inline void getAllMallocAddrs(std::vector<uint64_t> **malloc_addrs) {
  return get()->getAllMallocAddrs(malloc_addrs);
}

inline void stopTraceMallocFree() {
  return get()->stopTraceMallocFree();
}

inline void shutdownFree() {
  return get()->shutdownFree();
}

inline void turnonFree() {
  return get()->turnonFree();
}
