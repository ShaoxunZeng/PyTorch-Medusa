...
struct CudaMallocAsyncAllocator : public CUDAAllocator {
  ...
  void getAllMallocAddrs(std::vector<uint64_t> **malloc_addrs) override {
    TORCH_CHECK(
        false,
        "cudaMallocAsync does not yet support getAllMallocAddrs.");
  }
  ...
  void stopTraceMallocFree() override {
    TORCH_CHECK(
        false,
        "cudaMallocAsync does not yet support stopTraceMallocFree.");
  }
